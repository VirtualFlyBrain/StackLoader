#!/bin/bash
# Find OBJ files that match the label, move the original their VFB ref folder.
echo Converting...
source /partition/bocian/VFBTools/python-modules-2.6/bin/activate
if [ ! -f ./linkData.tsv ]
then
  echo "linkData.tsv file is missing! See linkDataExample.tsv for an example"
  ...
  do
    ...
    then
      ...
      nf=$(ls ${dirName}*${label}*.obj 2>/dev/null | wc -l)
      if [ $nf -gt 0 ]
      then
        if [ $nf -gt 1 ]
        then
          ...
          echo Processing ${ref}...
          file=$(ls ${dirName}*${label}*.obj 2>/dev/null)
          ...
          then
            ...
            mv -v ${file} ${imageDir}${first}/${last}/volume_man.obj
          fi
        fi
      else
